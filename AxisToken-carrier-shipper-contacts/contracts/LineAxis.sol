...
abstract contract Owned {
    ...
    event TransferredOwner(address indexed previousOwner, address indexed newOwner);
...
contract ERC20 is ERC20CoreBase, IERC20 {
    mapping(address => mapping(address => uint256)) private _allowed;

    event Approval(address indexed owner, address indexed spender, uint256 value);

    constructor(address to, uint256 value) public {
        if (value > 0) {
            _mint(to, value);
        }
    }
...
contract LineAxis is Owned, ERC20 {
    ...
    constructor(
        ...
        uint256 _total,
        bool _frozen
    ) public ERC20(msg.sender, _total) {
        name = _name;
        symbol = _symbol;
        decimals = _decimals;
        frozen = _frozen;
    ...
    function transfer(address to, uint256 value) public override onlyUnfreeze returns (bool) {
    ...
    function transferFrom(
        ...
        address to,
        uint256 value
    ) public override onlyUnfreeze returns (bool) {
