...
contract Executor is MultiOwnable {
    ...
    SUSDC public _sUsdcToken; // not IERC20 because of mint
    IERC20 public _usdcToken;
    IERC20 public _axisToken;
    IERC20 public _ethToken;
    ...
    event LoadPriceUpdated(uint256 newPrice);
    ...
    constructor(
        ...
        _sUsdcToken = SUSDC(sUsdcToken_);
        _axisToken = IERC20(axisToken_);
        _uniswapQuoter = IQuoter(uniswapQuoter_);
        _uniswapRouter = ISwapRouter(uniswapRouter_);
        _ethToken = IERC20(0x4200000000000000000000000000000000000006);
        _sUSDCPerLoad = sUSDCPerLoad_;
    }
    ...
    function payOrder(
        ...
        require(_sUsdcToken.balanceOf(payer) >= sUsdcAmount, "not enough sUSDC");
        require(_axisToken.balanceOf(payer) >= axisAmount, "not enough AXIS");
        require(_axisToken.allowance(payer, address(this)) >= axisAmount, "AXIS allowance too low");
        _sUsdcToken.transferFrom(payer, order, sUsdcAmount);
        _axisToken.transferFrom(payer, order, axisAmount);
    }
